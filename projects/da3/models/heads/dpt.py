...
"""DPT head for dense prediction (main head + optional sky head)."""
...
class DPT(nn.Module):
    ...
    def forward(self, feats: List[torch.Tensor], H: int, W: int,
                patch_start_idx: int, chunk_size: int = 8, **kwargs) -> dict:
    ...
    def _forward_impl(self, feats, H, W, patch_start_idx) -> TyDict[str, torch.Tensor]:
        B, _, C = feats[0].shape
...
# Building blocks
def _make_fusion_block(features, size=None, has_residual=True, groups=1, inplace=False):
    return FeatureFusionBlock(
        features=features, activation=nn.ReLU(inplace=inplace),
